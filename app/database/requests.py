...
async def set_account(tg_id, flag_admin=0, flag_vld=0):
    async with async_session() as session:
        account = await session.scalar(select(Account).where(Account.tg_id == tg_id))
        if not account:
            account = Account(tg_id=tg_id, flag_admin=flag_admin, flag_vld=flag_vld)
            session.add(account)
        else:
            account.flag_admin = flag_admin
            account.flag_vld = flag_vld
...
async def set_character(account, name):
    async with async_session() as session:
        session.add(Character(account=account, name=name, money='1000', game_state=0))
        await session.commit()
...
async def set_new_item_to_char_inv(new_items, inv_id):
    async with async_session() as session:
        await session.execute(update(Donat_shop).where(Donat_shop.id == inv_id).values(items=new_items))
        await session.commit()
...
async def delete_char_inv_by_id(inv_id):
    async with async_session() as session:
        await session.execute(delete(Character).where(Character.id_char == inv_id))
        await session.commit()
...
# tg_id, account, name
async def get_characters(id):
    async with async_session() as session:
        return await session.scalars(select(Character).where(Character.id == id))
...
async def get_inventory_by_char_id(char_id):
    async with async_session() as session:
        return await session.scalar(select(Character).where(Character.id == char_id))
